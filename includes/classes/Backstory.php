<?php
    class Backstory {
        public function get_backstory( $id, $type ) {
            if ($type == 'concept'){
                $query = "SELECT characterID, FROM_BASE64(concept_content) as content, concept_status as status,
            ...
            if ($type == 'backstory'){
                $query = "SELECT characterID, FROM_BASE64(backstory_content) as content, backstory_status as status,
                status.status_name, status.status_description, timestamp
                FROM ecc_backstory
