...
class Joomla {
	...
	public function get_joomla_users_by_group( $group_id, $event_id ) {
		...
		WHERE r.event_id = $event_id AND u.id in (SELECT user_id FROM jml_user_usergroup_map WHERE group_id IN (SELECT id FROM jml_usergroups WHERE id = $group_id UNION SELECT id FROM jml_usergroups WHERE parent_id = $group_id)) ORDER by name"
		);
		$users    = $stmt->execute();
