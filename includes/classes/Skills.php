...
class Skills {
	...
	public function get_skills_by_tree( $id, $tree ) {
		$stmt         = Database::$conn->prepare( "SELECT skill_id FROM ecc_char_skills where charID = $id ORDER BY skill_ID" );
	   $res           = $stmt->execute( [ $id ] );
	   $a_char_skills = $stmt->fetchAll( PDO::FETCH_ASSOC );
	   ...
	   $stmt          = Database::$conn->prepare( "SELECT label, skill_index, level FROM ecc_skills_allskills skills 
	   left join  ecc_skills_groups skillgroup on skillgroup.primaryskill_id = skills.parent
	   WHERE skills.skill_id IN ($s_skillid) AND skillgroup.siteindex = '$tree' " );
	   $res           = $stmt->execute();
	   $a_char_skills = $stmt->fetchAll( PDO::FETCH_ASSOC );
	   ...
	   $stmt       = Database::$conn->prepare( "SELECT type, skillgroup_siteindex, skillgroup_level, description FROM ecc_char_implants WHERE charID = ? AND status = 'active' AND type != 'flavour' AND skillgroup_siteindex = '$tree'" );
	   $res        = $stmt->execute( [ $id ] );
	   $a_implants = $stmt->fetchAll( PDO::FETCH_ASSOC );
