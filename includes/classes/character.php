...
class character
{
    ...
    public function get($id, $needle)
    {
        if ($needle == "card_id") {
            ...
            $stmt = database::$conn->prepare("SELECT * FROM ecc_characters where $needle = ? AND sheet_status = 'active' AND status NOT LIKE 'figurant%'");
            $res = $stmt->execute(array($id));
            $res = $stmt->fetch(PDO::FETCH_ASSOC);
        } else {
    ...
    function addCharacter($character)
    {
        ...
        if (!$check) {
            ...
            $res = $stmt->execute(array(
                0, $character_name, $card_id, $faction, $rank, $threat_assessment, $douane_disposition, $douane_notes, $bastion_clearance, $ICC_number, $bloodtype, $ic_birthday, $homeplanet
            ));
            ...
            $res = $stmt->execute(array($character_name, $faction, $rank, $threat_assessment, $douane_disposition, $douane_notes, $bastion_clearance, $ICC_number, $bloodtype, $ic_birthday, $homeplanet, $check["characterID"]));
